...
本文对应 v0.1.0；正式资产与最终检查记录以 GitHub Releases 为准。普通用户使用 Release 二进制。不要将 RC.2 包改名作为 v0.1.0。
...
3. 执行 `d2core version --json`：正式包 version=0.1.0，与 BUILD.json.version 一致；gitCommit、buildTime 一致，gitDirty=false。
...
维护者从远端固定完整提交的新干净 checkout 构建，固定 Go 1.27.1，执行打包工具 --version 0.1.0；版本标签为 v0.1.0。不复用开发产物、不覆盖旧 tag 或包。提交与 SHA256 在最终发布时填写到发布记录，不能预写虚构值。
