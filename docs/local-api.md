...
检查所有TCP监听、主游戏端口UDP，以及与本进程TCP同号且同地址族地址重叠的UDP；独立UDP本地端点不能证明服务用途，标记partial，不把正常出站端点当冲突。服务端点若占用其他未回收实例主端口预约，或与其他本轮重新核验的服务端点同协议/同端口/地址重叠，返回PORT_IN_USE/observe并撤销ready；不杀进程、不换端口，仍需显式stop。地址重叠指同族相同地址或任一为通配；跨族IPv6通配仅诊断不确定。partial可与ready同时出现，表示已确认的就绪证据成立，但不能宣称所有附加UDP冲突已排除。外部未管理进程、NAT和公网可达性不由该跨实例检查保证。
