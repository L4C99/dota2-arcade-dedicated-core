...
| 游戏版本 / 安装改动 | steam.inf: Client/ServerVersion 6933，SourceRevision 10999813，Sep 15 2026；确认 gameinfo 历史新增 GMS/Advertise，本轮备份后还原修改前文件，详见 M0 对照记录 | 待采集 |
...
| 客户端 / 网络路径 | 用户本地客户端通过 127.0.0.1 连接；run02/run03 均实际进房确认，默认 VAC 开启 | 进房客户端位置、云端映射及防火墙待确认 |
...
Windows 的地图、安装、无在用专服、端口选择权限及回环客户端已由用户补齐，实际进房也已确认。以下清单保留用于后续复验；当前缺少 Ubuntu 访问/资源/隔离输入。
